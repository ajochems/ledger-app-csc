...
const ux_menu_entry_t menu_main[] = {
    {NULL, NULL, 0, &C_icon_CSC, "Use wallet to", "view accounts", 33, 12},
...
void handleGetPublicKey(uint8_t p1, uint8_t p2, uint8_t *dataBuffer,
    ...
    addressLength = csc_public_key_to_encoded_base58(privateKeyData, 33, tmpCtx.publicKeyContext.address, sizeof(tmpCtx.publicKeyContext.address), 0, 0);
...
void handleSign(uint8_t p1, uint8_t p2, uint8_t *workBuffer,
    ...
    addressLength = csc_public_key_to_encoded_base58(txContent.destination, 20, tmpCtx.publicKeyContext.address, sizeof(tmpCtx.publicKeyContext.address), 0, 1);
    tmpCtx.publicKeyContext.address[addressLength] = '\0';
